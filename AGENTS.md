...
- Migrations use `github.com/golang-migrate/migrate/v4` with embedded `iofs` SQL files.
...
- Application SQL is fixed to `duf.files`; do not reintroduce configurable table names.
- PostgreSQL pool settings are optional config values with defaults: `max_open_conns: 8`, `max_idle_conns: 8`, `conn_max_lifetime: 30m`, `conn_max_idle_time: 5m`.
...
- Scanner records do not generate IDs; read generated IDs back from PostgreSQL for hash updates.
- Keep the migration runner on a separate database connection so closing the migrator does not close the main app pool.
